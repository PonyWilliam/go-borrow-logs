...
func (c *borrowLogsService) FindByLogID(ctx context.Context, in *Req_LogID, opts ...client.CallOption) (*RspLogs, error) {
	req := c.c.NewRequest(c.name, "BorrowLogs.FindByLogID", in)
	out := new(RspLogs)
	err := c.c.Call(ctx, req, out, opts...)
	if err != nil {
		return nil, err
...
func (h *borrowLogsHandler) FindByLogID(ctx context.Context, in *Req_LogID, out *RspLogs) error {
	return h.BorrowLogsHandler.FindByLogID(ctx, in, out)
}
